...
if(__name__=='__main__'):
    ...
    #print(np.amax(abs_h_f_emp))
    #MA_z_coefs = MA_z_coefs/np.amax(abs_h_f_emp)
    #AR_z_coefs = AR_z_coefs/np.amax(abs_h_f_emp)

    [_, h_f_emp] = dsp.freqz(MA_z_coefs/np.square(np.amax(abs_h_f_emp)), AR_z_coefs/np.square(np.amax(abs_h_f_emp)), 2*np.pi*f)
    abs_h_f_emp = np.abs(h_f_emp)
